...
outp_comb_anova <- capture_call(left_join(
  nice(m1s),
  nice(m9s),
  by = "Effect",
...
emm_o3 <- capture_call(summary(
  as.glht(update(pairs(emm_i1), by = NULL)),
  test = adjusted("free")
))
...
emm_o7 <- capture_call(update(
  contrast(emm_i2, des_c),
  by = NULL,
  adjust = "holm"
))
...
save(
  ...
  emm_o7,
  p1,
  file = "inst/extdata/output_mixed_vignette.rda",
  compress = "xz"
)
...
save(
  ...
  m7lrt,
  m9lrt,
  file = "development/freeman_models.rda",
  compress = "xz"
)
