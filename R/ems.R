...
ems <- function(design, nested = NULL, random = "") {
  ...
  if (!is.null(nested)) {
    ...
    formula <- paste(
      c(as.character(design)[2:1], paste(terms, collapse = "+")),
      collapse = ""
    )
  ...
  if (!is.null(random)) {
    ...
    mat[, which(colnames(mat) %in% random)][
      mat[,
        which(colnames(mat) %in% random)
      ] ==
        1
    ] <- ""
    mat[, which(!colnames(mat) %in% random)][
  ...
  for (effect in rownames(ems)) {
    for (varcomp in colnames(ems)) {
      ...
      if (!all(effectVec %in% subs[[varcomp]])) {
        ans <- ""
      }
      if (effect == "e" & varcomp == "e") {
        ans <- "1"
      }
