...
Again, we can pass this model directly to `afex_plot` (see panel A). However, the data is not aggregated within the groupin variable worker. If we want to aggregate the individual data points for the grouping factor, we need to pass both the name of the grouping variable (`Worker`) and the data used for fitting.
...
example_model <- rstanarm::stan_glmer(prob ~ size + period + (1|herd),
    ...
    chains = 2, cores = 1, seed = 12345, iter = 500)
b1 <- afex_plot(example_model, "period")
b2 <- afex_plot(example_model, "period", id = "herd", data = cbpp)
...
b3 <- afex_plot(mm, "Machine")
b4 <- afex_plot(mm, "Machine", id = "Worker", data = Machines)
save(b1, b2, b3, b4, file = "inst/extdata/plots_rstanarm.rda")
```
...
Some models are unfortunately not yet supported. For example models fit with the new and pretty cool looking [`GLMMadaptive`](https://cran.r-project.org/package=GLMMadaptive) package. The following unfortunately does not produce a reasonable plot. Hopefully in the future.
