...
afex_plot.afex_aov <- function(
  ...
  error <- match.arg(
    error,
    c("none", "model", "mean", "within", "CMO", "between")
  )
  dots <- list(...)
  if (length(dots) > 0) {
    warning(
  ...
  if ("y" %in% all_vars) {
    stop(
      "Variable 'y' cannot be used for plotting. Rename variable and refit.",
      call. = FALSE
    )
  }
  ...
  if (
    error %in%
      c("model", "mean", "between") &&
      all(c(x, trace) %in% within_vars)
  ) {
    warning(
      "Panel(s) show within-subjects factors, ",
      "but not within-subjects error bars.\n",
      'For within-subjects error bars use: error = "within"',
      call. = FALSE
    )
  } else if (
    error %in% c("within", "CMO") && all(c(x, trace) %in% between_vars)
  ) {
    warning(
      "Panel(s) show between-subjects factors, ",
    ...
    any(between_vars %in% c(x, trace)) &&
      any(within_vars %in% c(x, trace)) &&
      error != "none"
  ) {
    warning(
      "Panel(s) show a mixed within-between-design.\n",
...
afex_plot.mixed <- function(
  ...
  if (
    !(error %in% c("none", "model", "mean")) &
      (length(id) > 1)
  ) {
    stop(
      "When aggregating over multiple random effects,\n",
...
afex_plot.default <- function(
  ...
  if (missing(data)) {
    ...
    if (!exists("data", mode = "list")) {
      stop(
        "data could not be recovered. Please pass data via data argument.",
        call. = FALSE
      )
    }
  }
  ...
  if (
    ...
    stop(
      "When aggregating over multiple ids,\n",
      '       error has to be in: c("model", "mean", "none")',
      call. = FALSE
    )
  }
