...
test_that("mixed: Maxell & Delaney (2004), Table 16.6, p. 845", {
  ...
  expect_that(
    object = unname(round(sqrt(Matrix::diag(vcov(mixed2_orig$full_model))), 3)),
...
test_that("mixed: Maxell & Delaney (2004), Table 16.7, p. 851 (uses simple F!)", {
  ...
  expect_that(
    object = unname(round(sqrt(Matrix::diag(vcov(mixed3_orig$full_model))), 2)),
