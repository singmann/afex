...
If we only have a single random-effect grouping factor we can also use [`GLMMadaptive`](https://cran.r-project.org/package=GLMMadaptive). `GLMMadaptive` provides support for a range of interesting response distributions, such a zero-inflated or hurdle distributions. Importantly, `GLMMadaptive` approximates the integral in the model's log-likelihood function via the Gold-standard method, adaptive Gaussian quadrature. 
...
As before, to crate an `afex_plot()`, we need to pass both the original `data`, and pass the grouping factor (`site`) to the `id`-argument to ensure individual-level data points are aggregated appropriately.
